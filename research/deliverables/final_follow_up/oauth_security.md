...
This document catalogs discrepancies, contradictions, unclear areas, and specific questions requiring resolution before implementation.
...
**Question for Third Party**: Is the SAML signature configuration intentional for compatibility, or should we recommend enabling signatures in the implementation?
...
**Priority: LOW** - May not affect implementation if tokens are only used during OAuth callback.
...
- Should implementation encrypt at rest?
...
2. **Is the SAML signature disabled configuration intentional?**
   ...
   - Should implementation default to requiring signatures?
...
6. **Should the implementation fix the CSRF vulnerability?**
...
| Question | Token refresh behavior | **LOW** | May not affect implementation |
...
1. **Confirm OAuth CSRF fix is in scope**
2. **Document secure OAuth flow** as implementation target

### Short-term

3. **Investigate pending identity flow** - understand full user journey
4. **Decide on SAML signature policy** - secure by default vs. compatibility

### Long-term

5. **Consider encrypted token storage** in implementation
6. **Add security audit checklist** to OAuth implementation requirements
