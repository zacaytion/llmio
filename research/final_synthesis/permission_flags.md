...
This document provides the implementation-ready specification for permission flags in the Go rewrite of Loomio. It synthesizes findings from our research, third-party discovery, and direct source verification.
...
```go
// GroupPermissions represents all permission flags for a group
type GroupPermissions struct {
    // Member permissions (what regular members can do)
    MembersCanAddMembers       bool `db:"members_can_add_members"`
    MembersCanAddGuests        bool `db:"members_can_add_guests"`
    MembersCanAnnounce         bool `db:"members_can_announce"`
    MembersCanCreateSubgroups  bool `db:"members_can_create_subgroups"`
    MembersCanStartDiscussions bool `db:"members_can_start_discussions"`
    MembersCanEditDiscussions  bool `db:"members_can_edit_discussions"`
    MembersCanEditComments     bool `db:"members_can_edit_comments"`
    MembersCanDeleteComments   bool `db:"members_can_delete_comments"`
    MembersCanRaiseMotions     bool `db:"members_can_raise_motions"`

    // Admin permissions
    AdminsCanEditUserContent   bool `db:"admins_can_edit_user_content"`

    // Visibility flags (used in queries, not abilities)
    ParentMembersCanSeeDiscussions bool `db:"parent_members_can_see_discussions"`
}

// DefaultGroupPermissions returns the default permission values
func DefaultGroupPermissions() GroupPermissions {
    return GroupPermissions{
        MembersCanAddMembers:           false, // Restrictive default
        MembersCanAddGuests:            true,
        MembersCanAnnounce:             true,
        MembersCanCreateSubgroups:      false, // Restrictive default
        ...
        AdminsCanEditUserContent:       true,
        ParentMembersCanSeeDiscussions: false, // Restrictive default
    }
}
```
...
func NewAbility(user *User) *Ability {
    return &Ability{user: user}
}

// CanCreateDiscussion checks if user can create a discussion in the group
func (a *Ability) CanCreateDiscussion(group *Group) bool {
    if group == nil {
        return false
    }
    if a.isAdminOf(group.ID) {
        return true
    }
    return group.Permissions.MembersCanStartDiscussions && a.isMemberOf(group.ID)
}
...
func (a *Ability) CanEditComment(comment *Comment, discussion *Discussion, group *Group) bool {
    ...
    // Member editing own comment (if allowed)
    if a.isMemberOf(discussion.ID) &&
       comment.AuthorID == a.user.ID &&
       group.Permissions.MembersCanEditComments {
        return true
    }

    // Admin editing any comment (if allowed)
    if a.isAdminOf(discussion.ID) && group.Permissions.AdminsCanEditUserContent {
        return true
    }
...
// CanDeleteComment checks if user can delete a comment
func (a *Ability) CanDeleteComment(comment *Comment, discussion *Discussion, group *Group) bool {
    if discussion.ClosedAt != nil {
        return false
    }

    // Cannot delete if comment has replies
    if comment.RepliesCount > 0 {
        return false
    }

    // Admin can always delete
    if a.isAdminOf(discussion.ID) {
        return true
    }

    // Member deleting own comment (if allowed)
    return comment.AuthorID == a.user.ID &&
           a.isMemberOf(discussion.ID) &&
           group.Permissions.MembersCanDeleteComments
}

// CanCreatePoll checks if user can create a poll
func (a *Ability) CanCreatePoll(poll *Poll, discussion *Discussion, group *Group) bool {
    // No group = allowed (standalone poll)
    if group == nil {
        return true
    }
    ...
    if group.Permissions.MembersCanRaiseMotions && a.isMemberOf(group.ID) {
        return true
    }

    // Discussion guest with permission
    if discussion != nil &&
       group.Permissions.MembersCanRaiseMotions &&
       a.isGuestOf(discussion.ID) {
        return true
    }
...
### Discussion Query with Visibility

```go
// DiscussionQuery builds a query for visible discussions
func DiscussionQuery(userID int64, groupIDs []int64) string {
    return `
        SELECT d.* FROM discussions d
        JOIN groups g ON d.group_id = g.id
        WHERE
            -- Direct member
            d.group_id IN (SELECT group_id FROM memberships WHERE user_id = $1)
            -- Or parent member with visibility
            OR (g.parent_members_can_see_discussions = TRUE
                AND g.parent_id IN (SELECT group_id FROM memberships WHERE user_id = $1))
...
**Status:** DEPRECATED - Do NOT implement in Go
...
**Recommendation:** Exclude from Go implementation. The original Rails app has effectively abandoned this flag.
...
```go
// NullGroupPermissions returns permissions for no-group context
func NullGroupPermissions() GroupPermissions {
    return GroupPermissions{
...
**Note:** There is an inconsistency in the original `Null::Group` concern where `members_can_add_guests` appears in both `true_methods` and `false_methods`. The `false_methods` list is processed after `true_methods`, so `false` takes precedence. We use `false` above.
...
The Go serializer should match this output for API compatibility.
...
```go
var VersionedPermissionFields = []string{
    "parent_members_can_see_discussions",
    "members_can_add_members",
    "members_can_edit_discussions",
    "members_can_edit_comments",
...
```go
func (g *Group) ValidateParentMembersCanSeeDiscussions() error {
    if g.IsVisibleToPublic {
        return nil // Always valid if public
    }
    if g.Permissions.ParentMembersCanSeeDiscussions {
        if !g.IsVisibleToParentMembers {
            return errors.New("parent_members_can_see_discussions requires is_visible_to_parent_members")
        }
    }
    return nil
}
