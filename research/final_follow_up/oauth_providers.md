...
**Priority**: HIGH (Affects Go implementation of SSO-only deployments)
...
**Priority**: MEDIUM (Configuration validation for Go implementation)
...
- Should Go implementation default to more secure settings?
...
**Priority**: LOW (Cosmetic frontend issue, not affecting Go rewrite)
...
- Should Go schema include this column?
...
**Needed for Go**: Clarify if OAuth tokens are stored just for initial auth or for ongoing API calls.
...
**Needed for Go**: TDD approach requires understanding test patterns.
