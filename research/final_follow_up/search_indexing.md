...
## Implementation Requirements for Go

### 1. Search Index Population

```go
// Use sqlc with raw SQL for INSERT
const insertSearchDocumentSQL = `
INSERT INTO pg_search_documents (
    searchable_type, searchable_id, group_id, discussion_id,
    author_id, authored_at, content, ts_content, created_at, updated_at
)
SELECT $1, $2, $3, $4, $5, $6,
    regexp_replace(concat_ws(' ', $7, $8, $9), E'<[^>]+>', '', 'gi'),
    to_tsvector('simple', regexp_replace(concat_ws(' ', $7, $8, $9), E'<[^>]+>', '', 'gi')),
    now(), now()
`
```
...
```go
const searchSQL = `
SELECT searchable_type, searchable_id, ts_rank(ts_content, query) AS rank
FROM pg_search_documents, plainto_tsquery('simple', $1) AS query
...
Option A: Synchronous (match Rails behavior)
Option B: Async via River job (better write performance)
