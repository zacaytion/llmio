...
### River Job Definitions

```go
// Example: Close expired polls
...
func (j CloseExpiredPollsJob) Work(ctx context.Context) error {
    polls, _ := queries.GetLapsedPolls(ctx)
    for _, poll := range polls {
        PollService.Close(ctx, poll)
    }
    return nil
}
```
...
```go
// River queue priorities
riverClient.Config = river.Config{
