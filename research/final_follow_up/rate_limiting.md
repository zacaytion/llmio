...
**Note for Go Rewrite:** Document correct spellings and support both variants for backwards compatibility.
...
**Priority:** LOW (for rewrite, we control both sides)
...
1. **Define Canonical Rate Limits:** Our research had incorrect values. The Go implementation needs a clear specification derived from verified Rack::Attack limits.

2. **Redis Key Expiration Strategy:** Must implement TTL on counters. Cannot rely on manual reset.

3. **Error Response Standardization:** Go must return proper 429 with `Retry-After` header, not 500.

4. **Bot API Decision:** Need product decision on whether `/api/b*` endpoints need rate limiting in Go.

### Recommended Go Approach

```go
// Use go-chi/httprate or ulule/limiter with Redis backend
// Implement per-endpoint limits matching Rack::Attack configuration
