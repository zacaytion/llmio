...
- What happens when the last administrator tries to leave a group? System prevents this - group must have at least one admin.
- How does the system handle a user being invited to the same group multiple times? Prevents duplicate memberships, returns existing invitation or membership.
- What happens when a parent group is archived? Subgroups remain accessible but their parent relationship shows as archived.
- How are group handles validated? Must be unique, lowercase, URL-safe (alphanumeric and hyphens only), 3-100 characters.
- What happens when a group name is very long? Names are limited to 255 characters.
- What happens when inviting a non-existent user? System returns 404 Not Found with message "User not found".
...
- **SC-001**: POST /api/v1/groups responds in < 500ms p95 under normal load
...
- **TC-004**: Actor context MUST be passed via PostgreSQL session variable (`SET LOCAL app.current_user_id = '<user_id>'`) at the start of each transaction; triggers read this to populate the actor field in audit records
- **TC-005**: Last-admin protection MUST be enforced via a BEFORE UPDATE/DELETE trigger on `memberships` that prevents removing or demoting the last admin of any group
