...
SELECT * FROM data.memberships WHERE id = $1;

-- name: GetMembershipByGroupAndUser :one
-- Retrieves a specific user's membership in a group
SELECT * FROM data.memberships WHERE group_id = $1 AND user_id = $2;
...
SELECT m.* FROM data.memberships m
WHERE m.group_id = $1
  AND (
