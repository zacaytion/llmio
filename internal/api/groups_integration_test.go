...
func (s *testAPISetup) createTestGroupAndGetID(t *testing.T, token, name string) int64 {
	t.Helper()
...
func (s *testAPISetup) inviteMember(t *testing.T, adminToken string, groupID, userID int64) int64 {
	t.Helper()
...
func (s *testAPISetup) acceptInvitation(t *testing.T, token string, userID, groupID int64) {
	t.Helper()
...
// createSubgroup creates a subgroup and returns its ID.
func (s *testAPISetup) createSubgroup(t *testing.T, token string, parentID int64, name string) int64 {
	t.Helper()
...
// Test_CreateGroup_TableDriven is the main table-driven test for createGroup handler.
...
// Test_CreateGroup_HandleAutoGeneration tests handle auto-generation from name.
...
// Test_CreateGroup_HandleCollisionRetry tests automatic suffix on collision.
...
// Test_CreateGroup_CreatorBecomesAdmin verifies that the creator automatically becomes an admin.
...
// Test_UpdateGroup_PermissionFlags tests updating group permission flags.
...
// Test_UpdateGroup_NonAdmin tests that non-admins cannot update groups.
...
// Test_InviteMember_PermissionFlag tests that members_can_add_members flag is enforced.
...
// Test_InviteMember_AdminBypass tests that admins can invite even when members_can_add_members=false.
...
// Test_GetGroup_PermissionFlags tests that getGroup returns all permission flags.
...
// Test_CreateSubgroup_AdminCreates tests that an admin can create a subgroup.
...
// Test_CreateSubgroup_MemberWithPermission tests that a member with permission can create subgroups.
...
// Test_CreateSubgroup_MemberWithoutPermission tests that a member without permission cannot create subgroups.
...
// Test_CreateSubgroup_AdminBypass tests that admin can create subgroups even when permission is disabled.
...
// Test_ListSubgroups tests that listSubgroups returns child groups.
...
// Test_CreateSubgroup_SelfRefBlocked tests that a subgroup cannot be its own parent.
...
// Test_CreateSubgroup_InheritPermissions tests that subgroups can inherit parent permissions.
...
// Test_CreateSubgroup_DefaultPermissions tests that subgroups use defaults without inheritance.
...
// Test_ArchiveGroup_AdminArchives tests that an admin can archive a group.
...
// Test_ArchiveGroup_NonAdmin tests that non-admins cannot archive groups.
...
// Test_ListGroups_ExcludesArchived tests that archived groups are excluded by default.
...
// Test_ListGroups_IncludesArchived tests that archived groups can be included.
...
// Test_UnarchiveGroup tests that unarchive sets archived_at to NULL.
...
// Test_GetGroup_ArchivedAccessible tests that archived groups are still accessible by ID.
...
// Test_Subgroup_ArchivedParent tests that subgroups show parent_archived indicator.
...
// Test_HandleCaseInsensitiveConflict tests that handles are case-insensitive for uniqueness.
...
// Test_GetGroupByHandle_CaseInsensitive tests that group lookup by handle is case-insensitive.
...
// Test_UpdateGroup_ArchivedReturns409 tests that updating an archived group returns 409.
...
// Test_HandleValidation_BoundaryLengths tests handle length boundary cases.
...
// Test_CreateSubgroup_ArchivedParentReturns409 tests that creating a subgroup under an archived parent returns 409.
...
// Test_CreateSubgroup_SelfRefBlocked_API tests that the API prevents self-referencing at the API level.
...
// Test_GetGroup_ParentArchiveStatus tests that parent archive status is correctly reported.
...
// Test_GetGroup_ParentArchiveStatusUnknown_Documented documents the expected behavior when parent fetch fails.
...
// Test_ListGroupsByUserWithCounts_IntegrationTest tests the optimized query that returns groups with counts.
