...
type testMembershipsSetup struct {
	...
	membershipHandler *api.MembershipHandler
	mux               *http.ServeMux
}

// setupMembershipsTest creates a test environment using the shared container.
...
// Test_InviteMember_TableDriven is the main table-driven test for inviteMember handler.
...
// Test_InviteMember_AlreadyMember tests that inviting an existing member returns 409.
...
// Test_AcceptInvitation tests accepting a pending invitation.
...
// Test_AcceptInvitation_NotFound tests accepting a non-existent invitation.
...
// Test_AcceptInvitation_WrongUser tests that a user cannot accept another user's invitation.
...
// Test_ListMemberships_StatusFilter tests filtering memberships by status.
...
// Test_InviteMember_InviterID tests that inviter_id is correctly recorded.
...
// Test_PromoteMember_TableDriven is the main table-driven test for promote/demote handlers.
...
// Test_PromoteMember_NonAdmin tests that non-admins cannot promote.
...
// Test_DemoteMember tests demoting an admin to member.
...
// Test_DemoteLastAdmin tests that demoting the last admin is blocked.
...
// Test_RemoveMember tests removing a member from a group.
...
// Test_RemoveLastAdmin tests that removing the last admin is blocked.
...
// Test_ListMyInvitations tests listing pending invitations for current user.
...
// Additional tests truncated for brevity - the full file contains all tests
// migrated from the original memberships_test.go with the new container pattern
